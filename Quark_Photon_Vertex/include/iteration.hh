...
double update_accuracy(const unsigned int z_0, const unsigned int n_structs, const qtens_cmplx &a, unsigned int q_iter,
...
void a_iteration_step(const unsigned int n_structs, const qtens_cmplx &b, const double int_factors, unsigned int q_iter,
...
void b_iteration_step(const unsigned int n_structs, const qtens_cmplx &a, unsigned int q_iter, const double &q_sq,
...
void precalculate_K_kernel(const vec_double &y_grid, const unsigned int n_structs,
...
void iterate_a_and_b(const vec_double &q_grid, const vec_double &z_grid, const vec_double &k_grid,
    ...
    for (unsigned int q_iter = 0; q_iter < q_steps; q_iter++) {
        ...
        precalculate_K_kernel(y_grid, n_structs, qint1d, q_sq, z_grid, k_grid, K_prime);
        ...
        while (max_steps > current_step && current_acc > target_acc) {
            ...
            b_iteration_step(n_structs, a, q_iter, q_sq, z_grid, k_grid, b);

            std::cout << "  Calculated b_i...\n";

            a_iteration_step(n_structs, b, int_factors, q_iter, K_prime, z_grid, k_grid, a, qint2d);

            std::cout << "  Calculated a_i...\n";

            current_acc = 0.;
            current_acc = update_accuracy(z_0, n_structs, a, q_iter, current_acc, a_old);
